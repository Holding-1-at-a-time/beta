...
interface AIEstimationProps {
    ...
    uploadedFiles: { id: string; url: string }[];
}
...
export function AIEstimation({ vehicleDetails, selectedServices, customizations, uploadedFiles }: AIEstimationProps) {
    ...
    const aiAnalysis = useQuery(api.aiAnalysis.getLatest, tenant ? { tenantId: tenant._id } : null);
    const estimatedTotal = useQuery(api.estimates.calculateTotal,
        tenant && aiAnalysis ? { tenantId: tenant._id, analysisId: aiAnalysis._id } : null
    );
    ...
    }, [tenant, vehicleDetails, selectedServices, customizations, uploadedFiles, generateAnalysis, toast]);

    useEffect(() => {
        if (tenant && vehicleDetails.make && selectedServices.length > 0 && uploadedFiles.length > 0) {
            handleGenerateAnalysis();
        }
    }, [tenant, vehicleDetails, selectedServices, customizations, uploadedFiles, handleGenerateAnalysis]);

    if (!tenant) {
        return <Alert variant="destructive"><AlertTitle>Error</AlertTitle><AlertDescription>Tenant information not available</AlertDescription></Alert>;
    }
            ...
            <CardContent>
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        {estimatedTotal ? (
                            <p className="text-lg font-bold mt-1">${estimatedTotal.toFixed(2)}</p>
                        ) : (
